...
Corrigé pour correspondre exactement aux fonctions existantes dans service_views.py.
...
urlpatterns = [
    ...
    # ── Routes fixes coiffeur — IMPÉRATIVEMENT avant <uuid:pk>/ ────
    ...
    # ── Liste (GET) + Création (POST) ───────────────────────────────
    # liste_services gère GET et POST sur le même endpoint
    path('', service_views.liste_services, name='services-list'),

    # ── Détail (GET) + Mise à jour (PATCH/PUT) + Suppression (DELETE)
    # detail_service gère les 4 méthodes sur le même endpoint
    path('<uuid:pk>/', service_views.detail_service, name='services-detail'),

    # ── Activation / Désactivation (POST) ──────────────────────────
